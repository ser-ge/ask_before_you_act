...
def run_experiment():
    ...
    env = OracleWrapper(
        env,
        syntax_error_reward=cfg.syntax_error_reward,
    ...
    _, train_reward = train(
        env,
        agent,
        logger,
        memory=cfg.use_mem,
        n_episodes=cfg.N_eps,
        log_interval=cfg.train_log_interval,
        verbose=True,
    )
...
def plot_experiment(runs_reward, total_runs, window=25):
    ...
    avg_rnd = (
        pd.DataFrame(np.array(runs_reward[:total_runs])).T.rolling(window).mean().T
    )
    avg_good = (
        pd.DataFrame(np.array(runs_reward[total_runs:])).T.rolling(window).mean().T
    )
    ...
    sns.lineplot(
        ax=ax, x="variable", y="value", data=reward_rnd, legend="brief", label="Random"
    )
    sns.lineplot(
        ax=ax, x="variable", y="value", data=reward_good, legend="brief", label="Good"
    )
